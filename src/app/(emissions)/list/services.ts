export async function getEmissions(page = "1", search = "", ativos: string | string[] = "", ofertas: string | string[] = "") {
  ...
  const res = await fetch(`${process.env.BASE_API}/emissao/passivosoperacoes?${ofertasConditional}${ativosConditional}nome%5B0%5D=${search}&pagina=${page}&tamanhoPagina=20`)
